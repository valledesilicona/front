<template>
  <div :class="['modal', active ? 'is-active' : '']">
    <div class="modal-background" @click="closeModal()"></div>
    <form @submit.prevent="sendForm" class="modal-card">
      <header class="modal-card-head">
        <p class="modal-card-title">Crear Sala</p>
      </header>
      <section class="modal-card-body">
        <div class="field">
          <label class="label">Nombre del Creador</label>
          <div class="control">
            <input :disabled="loading" class="input" v-model="form.user" type="text" placeholder="Nombre"
                   required>
          </div>
        </div>

        <div class="field">
          <label class="label">Magnet Torrent Link</label>
          <div class="control">
            <input :disabled="loading" class="input" v-model="form.link" type="text" placeholder="Link"
                   required>
          </div>
        </div>

        <div class="field">
          <label class="label">Seleccionar Película</label>
          <div class="control">
            <div class="search">
              <input :disabled="loading" class="input" v-model="search" @input="getDBFilms(search)" type="text" placeholder="Buscar Películas"
                     required>
            </div>
            <div class="select is-multiple">
              <select :disabled="loading" v-model="form.film_id" multiple size="8" class="selectFilm">
                <option
                v-for="film in films"
                :key="film.id"
                :value="film.id"
                v-text="film.title"/>
              </select>
            </div>
          </div>
        </div>
      </section>
      <footer class="modal-card-foot">
        <button class="button is-inverted is-outlined create-room" :disabled="!validateForm" type="submit">
          {{ loading ? 'Cargando' : 'Crear' }}
        </button>
        <a class="button is-danger is-outlined" @click="closeModal()">Cancelar</a>
      </footer>
    </form>
  </div>
</template>

<script>
import { mapGetters, mapActions } from 'vuex'

export default {
  name: 'CreateRoom',
  props: ['active'],
  data () {
    return {
      form: {
        user: null,
        link: null,
        film_id: []
      },
      search: '',
      loading: false,
      popularFilms: []
    }
  },
  computed: {
    ...mapGetters({
      // films: 'getFilms',
      dbFilms: 'takeDBFilms'
    }),
    films () {
      return this.dbFilms.length > 0 ? this.dbFilms : this.popularFilms
    },
    validateForm () {
      return this.form.user && this.form.film_id.length !== 0 && this.form.link
    }
  },
  mounted () {
    this.getDBFilmPopular().then(response => {
      console.log(response)
      this.popularFilms = response.data.results
    })
  },
  methods: {
    ...mapActions(['createRoom', 'getDBFilms', 'getDBFilmPopular']),
    closeModal () {
      this.$emit('closeModal')
    },
    sendForm () {
      this.loading = true
      const film = {
        film_id: this.form.film_id[0],
        user: this.form.user,
        link: this.form.link
      }
      this.createRoom(film).then(response => {
        setTimeout(() => {
          this.loading = false
          const film = response.data.film
          this.$router.push('/cinema/' + film.port + '/' + film.user)
        }, 5000)
      }).catch(() => {
        this.loading = false
        this.closeModal()
      })
    }
  }
}
</script>

<style scoped>
  .modal-card, .modal-card-head, .modal-card-body, .modal-card-foot, .selectFilm {
    background: #1c212e;
    border: 0px;
    border-radius: 0px;
  }

  .search {
    padding: 0;
    padding-bottom: 3px;
    margin-bottom: 0;
  }

  .modal-card-title {
    height: 40px;
    color: #8ebcee;
  }

  .create-room {
    width: 100%;
    border-color: #477BB3;
    color: #477BB3;
    background: transparent;
  }

  .create-room:hover {
    color: white;
  }

  .field .label {
    color: #949cb0;
  }

  .control {
    background: linear-gradient(135deg, #8ebcee, #388fff);
    padding: 3px;
    margin-bottom: 10px;
  }

  .input {
    border-radius: 0px;
    background: #1c212e;
    border: 1px solid transparent;
    color: #949cb0;
  }

  .input::placeholder {
    color: #464d5f;
  }

  select, .select {
    width: 100%;
  }

  option {
    color: #949cb0;
  }
</style>
